...
static struct
{
	...
	// player
	bool NoClipEnabled;
	bool NoClipExperimentalEnabled;
	bool NoSpikesEnabled;
	bool NoHitboxEnabled;
...
void checkHacks(){
	...
	if (setting.NoClipEnabled){
	WriteBytes((void*)(gd::base + 0x20A23C), {0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0xE9, 0x72, 0x06, 0x00, 0x00});
	} else {
		WriteBytes((void*)(gd::base + 0x20A23C), {0x6A, 0x14, 0x8B, 0xCB, 0xFF, 0x15, 0x00, 0x00, 0x00, 0x00, 0x8D, 0xCB});
	}
	if (setting.NoHitboxEnabled){
	WriteBytes((void*)(gd::base + 0x20456D), {0xB8, 0x07, 0x00, 0x00, 0x00, 0x90});
	} else {
	...
	if (setting.NoClipExperimentalEnabled){
		WriteBytes((void*)(gd::base + 0x20A23C), {0xE9, 0x79, 0x06, 0x00, 0x00, });
		WriteBytes((void*)(gd::base + 0x1E9F6B), {0xE9, 0x13, 0x02, 0x00, 0x00, 0x90, });
		WriteBytes((void*)(gd::base + 0x1EA195), {0xE9, 0x22, 0x01, 0x00, 0x00, 0x90, });
	} else {
		WriteBytes((void*)(gd::base + 0x20A23C), {0x6A, 0x14, 0x8B, 0xCB, 0xFF, });
		WriteBytes((void*)(gd::base + 0x1E9F6B), {0x0F, 0x84, 0x12, 0x02, 0x00, 0x00, });
		WriteBytes((void*)(gd::base + 0x1EA195), {0x0F, 0x8B, 0x21, 0x01, 0x00, 0x00, });
	}
...
static void ShowPlayerHacks(){
	...
	ImGui::Checkbox("NoClip (Experimental)", &setting.NoClipExperimentalEnabled);
	if (ImGui::IsItemHovered())
	ImGui::SetTooltip("Makes the player invincible. Safe patch by mgostIH");
	
	ImGui::Checkbox("No Spikes", &setting.NoSpikesEnabled);
	if (ImGui::IsItemHovered())
	ImGui::SetTooltip("Ignores spike objects.");

	ImGui::Checkbox("No Hitbox", &setting.NoHitboxEnabled);
	if (ImGui::IsItemHovered())
...
static void ShowAboutWindow(){
	ImGui::Begin("About GDH", NULL, ImGuiWindowFlags_NoResize);
    ...
    ImGui::Text("GDH %s", VERSION);
    ImGui::Separator();
    ImGui::Text("By Alexandr Simonov");
    ImGui::Text("GDH is licensed\nunder the MIT License.");
	ImGui::Separator();
